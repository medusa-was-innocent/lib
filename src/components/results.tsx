...
function Cover({ book }: { book: Book }) {
  ...
  return (
    <div className="relative h-[104px] w-[72px] shrink-0 overflow-hidden rounded-[4px] bg-ink-800 shadow-[3px_5px_0_rgba(12,31,49,0.16)] ring-1 ring-black/10 transition-transform duration-300 group-hover:-translate-y-1 group-hover:-rotate-1">
      {showImg ? (
        <img
  ...
  items.push(
    {
      ...
      tint: "bg-royal-soft text-royal",
    },
    {
      label: "Anna's Archive",
      sub: "shadow-library aggregator search",
      href: annasUrl(book),
      icon: <IconGlobe width={15} height={15} />,
      tint: "bg-acc-soft text-acc-deep",
    },
    {
      label: "Google Books",
    ...
    {
      label: "Z-Library portal",
      sub: "singlelogin.re — official entry",
      href: zPortalUrl,
      icon: <LogoMark size={15} />,
      tint: "bg-ink-900/10 text-ink-900",
    }
  );
      ...
      {open && (
        <div className="animate-pop-in absolute right-0 top-full z-30 mt-2 w-[272px] rounded-xl border border-line bg-card p-1.5 shadow-[0_18px_50px_rgba(12,31,49,0.22)]">
          {items.map((it) => (
            ...
            >
              <span className={`mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-md ${it.tint}`}>
                {it.icon}
              </span>
    ...
    >
      ...
      <div className="col-span-2 flex items-center gap-2 sm:col-span-1 sm:flex-col sm:items-end sm:justify-between">
        ...
        <button
          ...
          onClick={copyCite}
          title="Copy citation"
          className="flex h-9 w-9 items-center justify-center rounded-lg border border-line text-faint transition-all hover:border-royal hover:text-royal active:scale-90"
        >
...
function PdfButton({ article, onToast }: { article: Article; onToast: (m: string) => void }) {
  ...
  return (
    <button
      type="button"
      onClick={click}
      className={`flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-bold transition-all active:scale-95 ${style}`}
    >
      {state === "loading" ? (
        <>
  ...
  return (
    <div className="flex flex-col items-center px-6 py-20 text-center">
      ...
      <button
        type="button"
        onClick={onReset}
        className="mt-6 flex items-center gap-2 rounded-lg bg-ink-900 px-5 py-2.5 text-sm font-bold text-paper transition-all hover:-translate-y-0.5 hover:bg-ink-800 active:scale-95"
      >
  ...
  return (
    <div className="flex flex-col items-center px-6 py-20 text-center">
      ...
      <button
        type="button"
        onClick={onRetry}
        className="mt-6 flex items-center gap-2 rounded-lg bg-rust px-5 py-2.5 text-sm font-bold text-white transition-all hover:-translate-y-0.5 hover:bg-[#a94e2b] active:scale-95"
      >
...
function Pagination({
  ...
  const btn =
    "flex h-9 min-w-9 items-center justify-center rounded-lg border border-line bg-card px-2 font-mono text-xs font-semibold text-body transition-all hover:-translate-y-0.5 hover:border-royal hover:text-royal disabled:pointer-events-none disabled:opacity-40";
    ...
    <div className="mt-8 flex flex-wrap items-center justify-center gap-2">
      ...
      {from > 1 && (
        <>
          ...
          {from > 2 && <span className="px-1 font-mono text-xs text-faint">…</span>}
        </>
      )}
      {nums.map((n) => (
        <button
          ...
          type="button"
          onClick={() => onPage(n)}
          className={`flex h-9 min-w-9 items-center justify-center rounded-lg px-2 font-mono text-xs font-bold transition-all ${
            n === page
      ...
      {to < pages && (
        <>
          {to < pages - 1 && <span className="px-1 font-mono text-xs text-faint">…</span>}
      ...
      <div className="relative mx-auto max-w-6xl px-5 sm:px-8">
        ...
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-1.5 font-mono text-xs text-faint" aria-live="polite">
              {loading
                ? mode === "books"
