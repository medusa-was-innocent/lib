...
/* ------------------------------------------------------------------ */
/*  Shared: Books / Articles segmented control                         */
/* ------------------------------------------------------------------ */
    ...
    return (
      <button
        ...
        className={`flex items-center gap-1.5 rounded-full font-semibold transition-all duration-200 ${
          ...
          active
            ? "bg-acc text-ink-950 shadow-[0_2px_10px_rgba(240,163,47,0.45)]"
            : dark
...
function CountUp({ to, label, sub }: { to: number; label: string; sub: string }) {
  ...
  return (
    <div ref={ref} className="group flex items-baseline gap-3 border-b border-ink-700/70 py-3 last:border-0">
      <span className="font-display text-3xl font-semibold leading-none text-paper transition-colors group-hover:text-acc">
        {val}
...
/* ------------------------------------------------------------------ */
/*  Shelf of classic spines — click a spine to search it               */
/* ------------------------------------------------------------------ */
      ...
      <div className="flex items-end justify-center gap-[5px] pb-[13px]">
        {SPINES.map((s, i) => (
          <button
            ...
            className="row-in group/spine relative overflow-hidden rounded-t-[3px] text-left transition-transform duration-300 ease-out hover:-translate-y-3 focus-visible:-translate-y-3 focus-visible:outline-2 focus-visible:outline-acc"
            style={{
              width: s.w,
              height: s.h,
...
export function Masthead({
  ...
  return (
    <header className="noise relative overflow-hidden bg-ink-900 text-paper">
      {/* ambient glows */}
      ...
      <div className="relative mx-auto flex max-w-7xl items-center justify-between px-5 py-5 sm:px-8">
        <a href="#top" className="flex items-center gap-3">
          ...
          <span className="font-display text-2xl font-semibold tracking-tight">
            <span className="italic text-acc">z</span>Library
          </span>
        </a>
        ...
        <div className="flex items-center gap-3">
          ...
          <a
            ...
            className="rounded-lg border border-ink-600 bg-ink-800/70 p-2 text-[#c6d4e2] transition-all hover:-translate-y-0.5 hover:border-acc hover:text-acc"
          >
      ...
      <div className="relative mx-auto grid max-w-7xl gap-12 px-5 pb-14 pt-8 sm:px-8 lg:grid-cols-[1.08fr_0.92fr] lg:items-center lg:pt-12">
        <div>
          ...
          >
            <div className="flex flex-1 items-center gap-3 rounded-xl border border-ink-600 bg-ink-950/70 px-4 shadow-[0_10px_30px_rgba(0,0,0,0.35)] transition-all focus-within:border-acc focus-within:shadow-[0_0_0_4px_rgba(240,163,47,0.15)]">
            ...
            <button
              type="submit"
              className="group flex h-[52px] shrink-0 items-center justify-center gap-2 rounded-xl bg-acc px-7 text-[15px] font-bold text-ink-950 shadow-[0_10px_26px_rgba(240,163,47,0.35)] transition-all hover:-translate-y-0.5 hover:bg-[#ffbd52] active:translate-y-0 active:scale-[0.98]"
            >
          ...
          <div className="row-in mt-4 flex flex-wrap items-center gap-2" style={{ animationDelay: "380ms" }}>
            ...
            {SUGGESTIONS.map((s) => (
              <button
                ...
                type="button"
                onClick={() => onSearch(s)}
                className="rounded-full border border-ink-600 bg-ink-800/60 px-3 py-1 text-xs text-[#b9c9da] transition-all hover:-translate-y-0.5 hover:border-acc hover:text-acc"
              >
                {s}
              </button>
            ))}
            ...
            <div className="row-in mt-3 flex flex-wrap items-center gap-2" style={{ animationDelay: "440ms" }}>
              ...
              {recent.map((r) => (
                <button
                  ...
                  type="button"
                  onClick={() => onSearch(r)}
                  className="rounded-full border border-royal/40 bg-royal/15 px-3 py-1 text-xs text-[#a9cbe8] transition-all hover:-translate-y-0.5 hover:border-acc hover:text-acc"
                >
                  {r}
                </button>
              ))}
...
/* ------------------------------------------------------------------ */
/*  Sticky mini-search (appears after scrolling past the masthead)     */
/* ------------------------------------------------------------------ */
...
export function StickyBar({
  ...
  return (
    <div
      className={`fixed inset-x-0 top-0 z-50 border-b border-ink-700 bg-ink-950/95 backdrop-blur transition-transform duration-300 ${
    ...
    >
      <div className="mx-auto flex max-w-7xl items-center gap-3 px-5 py-2.5 sm:px-8">
        ...
        >
          ...
          <button
            type="submit"
            className="h-9 rounded-lg bg-acc px-4 text-sm font-bold text-ink-950 transition-transform hover:bg-[#ffbd52] active:scale-95"
          >
            Go
          </button>
        </form>
